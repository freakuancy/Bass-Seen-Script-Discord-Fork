...
#Discord Edit fork 1.4.3 by Freakuancy freakuancy@gmail.com
...
# 
#DISCORD EDIT:
#    This has been edited to work with reactiflux-style discord 
#    to IRC bridges. !seen and !seentest will respond to users
#    querying the bot from discord.
###Parameters:
...
###################################################################
###################################################################
###### Edits for discord bridge users in flagrant disregard #######
###### of the warning above.                                #######
###################################################################
################################################################### 

bind pubm - "% % [string trim $bs(cmdchar)]seen*" bs_discordreq1
bind pubm - "% % [string trim $bs(cmdchar)]seennick*" bs_discordreq2

proc bs_discordreq1 {nick uhost hand chan args} {bs_discordreq $nick $uhost $hand $chan $args 0}
proc bs_discordreq2 {nick uhost hand chan args} {bs_discordreq $nick $uhost $hand $chan $args 1}

proc bs_discordreq {nick uhost hand chan args no} {
  global discbot
  if {[string equal $nick $discbot]} {
    ...
    set data [bs_filt [string trimright [lindex $args 2] ?!.,]]
    set discordnick [string range [bs_filt [string trimright [lindex $args 0] ?!.,]] 1 end-1]
    if {[string tolower $nick] == [string tolower $data] } {puthelp "$target :$discordnick, go look in a mirror." ; return $bs(logqueries)}
    if {[string tolower $data] == [string tolower $botnick] } {puthelp "$target :$discordnick, I'm right here.  Quit wasting my time!" ; return $bs(logqueries)}
    if {[onchan $data $chan]} {puthelp "$target :$discordnick, $data is right here!" ; return $bs(logqueries)}
    set output [bs_output $chan $discordnick $data $no] ; if {$output == 0} {return 0}
